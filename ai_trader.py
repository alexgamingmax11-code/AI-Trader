...
# LLM API — uses local proxy by default (Omniroute at localhost:20128)
# To use Anthropic directly, set ANTHROPIC_BASE_URL to https://api.anthropic.com
ANTHROPIC_API_KEY = os.environ.get("ANTHROPIC_API_KEY") or os.environ.get("ANTHROPIC_AUTH_TOKEN", "sk-c45ac5c4a7cfcd8e-18f3bc-7a4ab126")
ANTHROPIC_BASE_URL = os.environ.get("ANTHROPIC_BASE_URL", "http://localhost:20128/v1")
LLM_MODEL = os.environ.get("LLM_MODEL", "auto/best-reasoning")
LLM_MAX_TOKENS = 16000  # Uncapped for max reasoning power
...
CANDLE_CONFIGS = [
    {"interval": 60,   "limit": 72, "label": "1h"},    # 3 days of hourly (structure/trend)
    {"interval": 15,   "limit": 48, "label": "15m"},   # 12h of 15-min (momentum/patterns)
    {"interval": 5,    "limit": 24, "label": "5m"},    # 2h of 5-min (raw candles for entry)
    {"interval": 1,    "limit": 15, "label": "1m"},    # 15m of 1-min (raw candles for timing)
]
...
def format_candles_for_llm(candles, symbol, label):
    ...
    max_per_tf = {"1h": 24, "15m": 24, "5m": 12, "1m": 10}
...
Analyze all three timeframes together. Look for:
...
- position_size_eur: only for BUY, max EUR {portfolio['starting_capital_eur'] * POSITION_SIZE_PCT:.2f}"""
    ...
    try:
        ...
        # Find the text block (thinking models put thinking first)
        text = ""
...
def send_trade_email(action, symbol, price, reasoning, portfolio):
    ...
    if action == 'BUY':
        subject = f"🟢 BUY: {symbol} @ EUR {price:.2f}"
    else:
        subject = f"🔴 SELL: {symbol} @ EUR {price:.2f}"
